...
const JobPostCreation = ({ onClose }) => {
  ...
  const { mutate: createJobPost, isPending } = useMutation({
  ...
  const validateForm = () => {
    ...
    if (!formData.applicationUrl && !formData.applicationEmail) {
      newErrors.application = 'Either application URL or email is required';
    }
